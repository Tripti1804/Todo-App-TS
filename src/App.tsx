...
function App() {
  ...
  const handleAddTodo = (): void => {
    ...
    const updatedTodoArr: TodoItem[] = [...allTodos, newTodoItem];
  ...
  const handleDeleteTodo = (index: number): void => {
    const reducedTodo: TodoItem[] = [...allTodos];
  ...
  useEffect(() => {
    ...
    if (savedCompletedTodo.length) {
      setCompletedTodos(savedCompletedTodo);
    }
  }, []);

  const handleEdit = (ind: number, item: TodoItem): void => {
    setCurrentEdit(ind);
  ...
  const handleUpdateToDo = (): void => {
    if (currentEditedItem && currentEdit !== null) {
